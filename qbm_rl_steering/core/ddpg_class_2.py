...
class ReplayBuffer:
    ...
    def sample(self):
        ...
        states, actions, rewards, next_states, done = map(np.asarray,
                                                          zip(*sample))
...
class Agent:
    ...
    def target_update(self):
        ...
        for i in range(len(actor_weights)):
            t_actor_weights[i] = args.tau * actor_weights[i] + (1 - args.tau) * \
                                 t_actor_weights[i]

        for i in range(len(critic_weights)):
            t_critic_weights[i] = args.tau * critic_weights[i] + (
    ...
    def ou_noise(self, x, rho=0.15, mu=0, dt=1e-1, sigma=0.2, dim=1):
        return x + rho * (mu - x) * dt + sigma * np.sqrt(dt) * np.random.normal(
            size=dim)
    ...
    def train(self, max_episodes=1000):
        for ep in range(max_episodes):
            ...
            while not done:
                ...
                self.buffer.put(state, action, (reward + 8) / 8, next_state,
                                done)
                bg_noise = noise
